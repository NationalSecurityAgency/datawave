...
The branch file must match an entry in `SUPPORTED_RUNNER_TYPES`; runner labels do
not need to follow a particular naming pattern. If it is absent, empty, malformed,
or unsupported, CI falls back to `RUNNER_TYPE`. The `configure-runner` job reports
the selected source, resolved runner, and fallback reason in its log and job summary.
