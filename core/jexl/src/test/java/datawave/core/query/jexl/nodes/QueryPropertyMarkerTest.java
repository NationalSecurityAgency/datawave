...
import static datawave.core.query.jexl.JexlASTHelper.parseJexlQuery;
...
public class QueryPropertyMarkerTest {
    ...
    public void getDroppedExpression() {
        ...
        JexlNode marked = QueryPropertyMarker.create(new DroppedExpression(JexlStringBuildingVisitor.buildQuery(source)).getJexlNode(), DROPPED);
        assertEquals(expected, JexlStringBuildingVisitor.buildQueryWithoutParse(marked));

        expected = "((_Drop_ = true) && ((_Query_ = 'FOO =~ \\'ba.*\\'') && (_Reason_ = 'my reason')))";
        marked = QueryPropertyMarker.create(new DroppedExpression(JexlStringBuildingVisitor.buildQuery(source), "my reason").getJexlNode(), DROPPED);
        assertEquals(expected, JexlStringBuildingVisitor.buildQueryWithoutParse(marked));
    }
}
