...
public class Compare extends AbstractEvaluationPhaseFunction {
    ...
    public void validate() throws IllegalArgumentException {
        ...
        if (!operators.contains(parameterList.get(1))) {
            throw new IllegalArgumentException("#COMPARE function requires a valid op arg: " + operators);
        }

        try {
            Mode.valueOf(parameterList.get(2).toUpperCase());
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException("#COMPARE function requires a valid mode arg: " + Arrays.toString(Mode.values()));
        }
    }
