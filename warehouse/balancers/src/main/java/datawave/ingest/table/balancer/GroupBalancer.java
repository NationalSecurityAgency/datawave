...
/**
 * A balancer that evenly spreads groups of tablets across all tablet servers. This balancer accomplishes the following two goals :
 *
 * <ul>
 * <li>Evenly spreads each group across all tservers.</li>
 * <li>Minimizes the total number of groups on each tserver.</li>
 * </ul>
 *
 * <p>
...
public abstract class GroupBalancer extends TabletBalancer {
    ...
    public long balance(SortedMap<TServerInstance,TabletServerStatus> current, Set<KeyExtent> migrations, List<TabletMigration> migrationsOut) {
        ...
        // numInGroup/numTservers. Extra tablets are any tablets more than the number of expected for a given group. If numIngroup % numTservers > 0, then a
        // tserver may have one extra tablet for a group.
        //
        // Assume we have 4 tservers and group A has 11 tablets.
        // * expected tablet : group A is expected to have 2 tablets on each tserver
        // * extra tablets : group A may have an additional tablet on each tserver. Group A has a total of 3 extra tablets.
        //
        // This balancer also events out the extra tablets across all groups. The terminology extraExpected and extraExtra is used to describe these tablets.
        // ExtraExpected is totalExtra/numTservers. ExtraExtra is totalExtra%numTservers. Each tserver should have at least expectedExtra extra tablets and at
        // most one extraExtra tablets. All extra tablets on a tserver must be from different groups.
        //
        ...
        // possibly have 2 tablets from a group. Below is an illustration of an ideal balancing of extra tablets. To understand the illustration, the first
        // column shows tserver T1 with 2 tablets from G1, 1 tablet from G2, and two tablets from G3. EE means empty, put it there so eclipse formatting
        // would not mess up table.
        //
        // T1 | T2 | T3 | T4 | T5 | T6
        // ---+----+----+----+----+----
        ...
        // G3 | G3 | G3 | G3 | G3 | G3 <-- expected tabelts for group 3
        //
        // Do not want to balance the extra tablets like the following. There are two problems with this. First, extra tablets are not event spread. Since
        // there are a total of 9 extra tablets, every tserver is expected to have at least one extra tablet. Second tserver T1 has two extra tablets for
        // group G1. This violates the principal that a tserver can only have one extra tablet for a given group.
        //
        // T1 | T2 | T3 | T4 | T5 | T6
        // ---+----+----+----+----+----
    ...
    private void balanceExpected(Map<TServerInstance,TserverGroupInfo> tservers, Moves moves) {
        ...
        for (String group : groupDeficits.keySet()) {
            Collection<TserverGroupInfo> deficitServers = groupDeficits.get(group);
            for (TserverGroupInfo deficitTsi : deficitServers) {
                int numToMove = deficitTsi.getExpectedDeficits().get(group);
                ...
                while (numToMove > 0) {
                    ...
                    moves.move(group, transfer, surplusTsi, deficitTsi);
                    if (moves.size() >= getMaxMigrations()) {
                        return;
                    }
                }
    ...
    class MetadataLocationProvider implements Iterable<Pair<KeyExtent,Location>> {
        ...
        public Iterator<Pair<KeyExtent,Location>> iterator() {
            try {
                Scanner scanner = new IsolatedScanner(getConnector(configuration).createScanner(MetadataTable.NAME, Authorizations.EMPTY));
        ...
        private Connector getConnector(ServerConfiguration conf) throws AccumuloSecurityException, AccumuloException {
            ...
            return instance.getConnector(credentials.getPrincipal(), credentials.getToken());
        }
    }
}
