...
ADDJARS=$THIS_DIR/$DATAWAVE_INGEST_CORE_JAR,$THIS_DIR/$COMMON_UTIL_JAR,$THIS_DIR/$JODA_TIME_JAR,$THIS_DIR/$DATAWAVE_CORE_JAR,$THIS_DIR/$DATAWAVE_WS_CLIENT_JAR

$WAREHOUSE_ACCUMULO_BIN/accumulo -add $ADDJARS nsa.datawave.ingest.util.GenerateEdgeKeyVersionCache -Dfs.default.name=file:/// $RUN_OPS $USERNAME $PASSWORD ${METADATA_TABLE_NAME} $WAREHOUSE_INSTANCE_NAME $WAREHOUSE_ZOOKEEPERS
