...
DATAWAVE_INGEST_CORE_CONF_JAR=../../lib/datawave-ingest-core-configuration-$INGEST_VERSION.jar
POLLER_JAR=../../lib/datawave-poller-$INGEST_VERSION.jar
DATAWAVE_COMMON_JAR=../../lib/datawave-common-$INGEST_VERSION.jar
COMMON_UTIL_JAR=$(findWebserviceJar Common-Util)
COMMON_JAR=$(findWebserviceJar Common)
DATAWAVE_CORE_JAR=$(findJar datawave-core)
DATAWAVE_WS_QUERY_JAR=$(findWebserviceJar Query)
DATAWAVE_WS_CLIENT_JAR=$(findWebserviceJar Client)
COMMONS_LANG_JAR=$(findJar commons-lang)
COMMONS_LANG3_JAR=$(findJar commons-lang3)
COMMONS_COLLECTIONS_JAR=$(findJar commons-collections)
...
if [[ "$INCLUDE_POLLER_LIBS" != "" ]]; then
    CLASSPATH=${CLASSPATH}:${JPOLLER_JAR}
    CLASSPATH=${CLASSPATH}:${COMMONS_CLI_JAR}
    CLASSPATH=${CLASSPATH}:${COMMONS_LOGGING_JAR}
    CLASSPATH=${CLASSPATH}:${COMMONS_CODEC_JAR}
    CLASSPATH=${CLASSPATH}:${HORNETQ_CLIENT_JAR}
    CLASSPATH=${CLASSPATH}:${HORNETQ_COMMONS_JAR}
    CLASSPATH=${CLASSPATH}:${HORNETQ_NETTY_JAR}
fi
