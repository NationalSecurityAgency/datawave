...
declare -i PIPELINE_COUNT=$((INGEST_FIVE_MIN_JOBS))
...
while [ 1 == 1 ]; do
  ...
  for (( pipeline=1; pipeline <= $((PIPELINE_COUNT)); pipeline=$((pipeline+1)) )); do
    declare -i PIPELINE_JOB_COUNT=`ps -ef | grep five-min-execute.sh | grep " -pipelineId ${pipeline} " | wc -l`
    ...
    if [[ $((PIPELINE_TOTAL_COUNT)) == 0 ]]; then
      if [[ "$MAPRED_INGEST_OPTS" =~ "-markerFileLIFO" ]]; then
        flag_files=`find ${FLAG_DIR}/ -regextype posix-egrep -regex ".*_(live|fivemin)_.*\.flag" -printf "%AY%Aj%AT %p\n" | sort -r | head -1 | awk '{print $2}'`
      else
      ...
      for first_flag_file in $flag_files; do
        if [[ -a $first_flag_file ]]; then
            ...
            $BIN_DIR/five-min-execute.sh $BIN_DIR ${first_flag_file}.inprogress $LOG_DIR $FLAG_DIR -pipelineId $pipeline $EXTRA_ARGS &
        fi
      done
    fi
  done
