...
public class SplittableRFileInputFormatTest {
    ...
    public void testHugeSplits() throws IOException, InterruptedException {
        ...
        try {
            ...
            List<InputSplit> splits = SplittableRFileInputFormat.getSplits(config, new Path(tmpFile.toString()));
            assertEquals(2, splits.size());
            verifySplit((RFileSplit) splits.get(0), 0, 2, new Key(String.format("%08x", 1)), new Key(String.format("%08x", 2)), 11000);
            verifySplit((RFileSplit) splits.get(1), 2, 1, new Key(String.format("%08x", 3)), new Key(String.format("%08x", 3)), 1000);
        } finally {
            if (tmpFile.exists()) {
                tmpFile.delete();
            }
    ...
    public void testMegaSplit() throws IOException, InterruptedException {
        ...
        try {
            ...
            verifySplit((RFileSplit) splits.get(0), 0, 127, new Key(String.format("%08x", 1)), new Key(String.format("%08x", 1)), 1000000);
            verifySplit((RFileSplit) splits.get(1), 127, 1, new Key(String.format("%08x", 2)), new Key(String.format("%08x", 2)), 1000);
            verifySplit((RFileSplit) splits.get(2), 128, 1, new Key(String.format("%08x", 3)), new Key(String.format("%08x", 3)), 1000);
        } finally {
            if (tmpFile.exists()) {
                tmpFile.delete();
            }
    ...
    private void verifySplit(RFileSplit split, int startBlk, int blkCount, Key first, Key last, int keyCount) throws IOException, InterruptedException {
