                            ...
                            if (character == '(') {
                                ...
                                if (c < remainder.length() - 1 && remainder.charAt(c + 1) == '?') {
                                    ...
                                    if (flagRegexPattern.matcher(value).matches()) {
                                        ...
                                        throw new JavaRegexParseException("Invalid Regular Expression: does not match a known non-capturing group construct: "
                                                        + value, column);
                                    }
                                } else {
    ...
    public String getZeroPadIpRegex() throws JavaRegexParseException {
        ...
        for (RegexPart[] tuple : tuples) {
            if (tuple.length != 1 || !ignore.contains(tuple[0])) {
                if (inTuple) {
                    throw new JavaRegexParseException("Currently cannot handle tuples that cross over group boundaries: " + previousTuple + " and "
                                    + getRegex(tuple), -1);
                }
                inTuple = true;
