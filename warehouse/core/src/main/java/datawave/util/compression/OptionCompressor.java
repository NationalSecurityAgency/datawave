 ...
 * <p>General process:</p>
 * <ol>
 *     <li><b>Compression</b>: Original Data → Compress with Algorithm (selected via {@link CompressionMethod}) → Encode in Base64 (except {@code NONE}) → Return as String</li>
 *     <li><b>Decompression</b>: Input String (Base64 if compressed; plain text if {@code NONE}) → Decode Base64 (if applicable) → Decompress with Algorithm → Return Original Data</li>
 * </ol>
 *
 * <p>Base64 encoding ensures that compressed data can be safely stored or transferred as text.
 * For {@link CompressionMethod#NONE} the data is returned as-is and is not Base64-encoded.</p>
 */
public class OptionCompressor {
     ...
     *
     * <p>Order of operations (by method):</p>
     * <ul>
    ...
    /**
     * GZIP implementation reference:
     * 1) String → bytes via charset
     * 2) Write to GZIPOutputStream (must close to finalize)
     * 3) Base64-encode compressed bytes
     */
    ...
    /**
     * BZIP2 implementation (mirrors GZIP flow):
     * 1) String → bytes via charset
     * 2) Write to BZip2CompressorOutputStream (close to finalize)
     * 3) Base64-encode compressed bytes
     */
    ...
    /**
     * "7-Zip" via LZMA stream implementation:
     * 1) String → bytes via charset
     * 2) Write to LZMACompressorOutputStream (close to finalize)
     * 3) Base64-encode compressed bytes
     *
     * Note: This uses the LZMA compressor stream from Apache Commons Compress,
     * which is the algorithm used by 7-Zip; it is not a .7z archive container.
     */
    private String compress7ZIP(final String data, final Charset charset) {
        final byte[] input = data.getBytes(charset);

        try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
             LZMACompressorOutputStream lzma = new LZMACompressorOutputStream(baos)) {
    ...
    /**
     * GZIP implementation reference for decompression:
     * 1) Base64-decode to compressed bytes
     * 2) Read via GZIPInputStream into buffer
    ...
    /**
     * BZIP2 decompression (mirrors GZIP flow):
     * 1) Base64-decode to compressed bytes
     * 2) Read via BZip2CompressorInputStream into buffer
     * 3) Collect into ByteArrayOutputStream → String via charset
     */
    ...
    /**
     * "7-Zip" via LZMA stream decompression (mirrors GZIP/BZIP2 flow):
     * 1) Base64-decode to compressed bytes
     * 2) Read via LZMACompressorInputStream into buffer
     * 3) Collect into ByteArrayOutputStream → String via charset
     *
     ...
     * </ul>
     *
     * <p>This is the reverse of {@link #compress(String, CompressionMethod, Charset)}. For all non-{@code NONE} methods,
     * the input should be the Base64 text returned by the corresponding {@code compress(...)} call.</p>
     */
