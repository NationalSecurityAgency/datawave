...
public class MultiRFileOutputFormatter extends FileOutputFormat<BulkIngestKey,Value> {
    ...
    protected List<CompletableFuture<Void>> loadPlanFutures = new LinkedList<>();
            ...
            if (file != null) {
                if (writer.isLoadPlanning()) {
                    ...
                    loadPlanFutures.add(CompletableFuture
                       .supplyAsync(() -> createLoadPlan(usedWriterPaths.get(key), writer.rows, table))
                       .thenAccept(p -> addPlan(p.getKey(), p.getValue())));
                    // @formatter:on
                }
                writer.close();
    ...
    /**
     * Creates a {@link LoadPlan} list for the given RFile by mapping its row values to the relevant KeyExtents from the given table. A list is required here
     * due to constraints on how Accumulo builds LoadPlan instances (see return value note below)
     *
     * @param rfile
     *            RFile path
     * @param rfileRows
     ...
     * @return Table name-LoadPlan pair for the RFile.
     *         <p>
     *         Note that the LoadPlan API doesn't currently provide a way to add {@link LoadPlan.Destination} entries (i.e., key extents) to an existing
     *         LoadPlan instance. The only way to construct the entire plan is to create a list of LoadPlans, one for each extent, and then use
     *         {@link LoadPlan.Builder#addPlan(LoadPlan)} after the fact to construct a single consolidated plan from that list. This consolidation is performed
     *         later in {@link #writeLoadPlans(TaskAttemptContext)}
     *         </p>
     */
    protected Pair<String,List<LoadPlan>> createLoadPlan(Path rfile, SortedSet<Text> rfileRows, String tableName) {
        List<LoadPlan> destinations = new LinkedList<>();
        if (rfileRows != null && !rfileRows.isEmpty()) {
            List<Text> tableSplits;
            try {
                tableSplits = getSplitsCache(conf).getSplits(tableName);
            } catch (Exception e) {
            ...
            if (tableSplits == null || tableSplits.isEmpty()) {
                ...
                destinations.add(LoadPlan.builder().loadFileTo(
                   rfile.getName(), RangeType.FILE, ke.prevEndRow, ke.endRow).build());
            } else {
                sanityCheckSplitsList(tableSplits);
                ...
                rfileRows.stream().map(row -> findKeyExtent(row, tableSplits))
                   ...
                   .forEach(ke -> destinations.add(LoadPlan.builder().loadFileTo(
        ...
        if (destinations.isEmpty()) {
            log.warn("Load planning is enabled, but no load plans were created for " + rfile);
        }
        return new ImmutablePair<>(tableName, destinations);
    }
     ...
     *
     * @param splits
     *            sorted splits list whose implementation is assumed to provide fast random access (i.e., {@link java.util.RandomAccess})
     * @param lookup
     *            row for which we want to find the ceiling
     * @return index of the first split &ge; lookup, or -1 if lookup &gt; than the last split
     */
    static int findCeiling(List<Text> splits, Text lookup) {
        int begin = 0;
    ...
    private static void sanityCheckSplitsList(List<Text> splits) {
        // @formatter:off
        // TODO: Perhaps modify TableSplitsCache client interface to be more explicit about this
        if (!(splits instanceof RandomAccess)) {
            throw new IllegalArgumentException("Yikes! We expected splits list to implement fast random access. Found "
               + splits.getClass() + " instead. Please investigate, and disable load planning temporarily");
        }
        // @formatter:off
    }
            ...
            public void close(TaskAttemptContext context) throws IOException, InterruptedException {
                for (Map.Entry<String,SizeTrackingWriter> entry : writers.entrySet()) {
                    ...
                    if (writer.isLoadPlanning()) {
                        ...
                        // @formatter:off
                        loadPlanFutures.add(CompletableFuture
                           .supplyAsync(() -> createLoadPlan(file, writer.rows, table))
                           .thenAccept(p -> addPlan(p.getKey(), p.getValue()))
                        );
                        // @formatter:on
                    }
