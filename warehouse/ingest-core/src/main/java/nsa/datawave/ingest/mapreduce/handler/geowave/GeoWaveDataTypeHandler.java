package nsa.datawave.ingest.mapreduce.handler.geowave;
...
import nsa.datawave.ingest.metadata.GeoWaveMetadata;
        ...
        for (final AttributeDescriptor attrib : simpleFeatureType.getAttributeDescriptors()) {
            ...
            if (values.size() == 1) {
                ...
                if (String.class.isAssignableFrom(binding)) {
                    ...
                    for (final nsa.datawave.data.type.Type<?> type : helper.getDataTypes(attrib.getLocalName())) {
