package nsa.datawave.util.flag;
...
import nsa.datawave.util.flag.processor.DateFlagDistributor;
import nsa.datawave.util.flag.processor.DateFolderFlagDistributor;
import nsa.datawave.util.flag.processor.DateUtils;
import nsa.datawave.util.flag.processor.FlagDistributor;
import nsa.datawave.util.flag.processor.SimpleFlagDistributor;
import nsa.datawave.util.flag.processor.SizeValidator;
import nsa.datawave.util.flag.processor.UnusableFileException;
...
public class FlagMaker implements Runnable, Observer, SizeValidator {
    ...
    private static void printUsage() {
        ...
        System.out.println("nsa.datawave.ingest.flag.FlagMaker -flagConfig [path to xml config]");
    ...
    void writeFlagFile(final FlagDataTypeConfig fc, Collection<InputFile> inFiles) throws IOException {
        ...
        try {
            ...
            log.info("Creating flag file" + baseName + ".flag" + " for data type " + fc.getDataName() + " containing " + moved.size() + " files");
            ...
            if (estSize != sb.length()) {
                log.error("Estimated size is not accurate: " + estSize + " vs " + sb.length());
            }
        ...
        if (s.startsWith("kick")) {
            ...
            for (FlagDataTypeConfig cfg : fmc.getFlagConfigs()) {
                if (cfg.getDataName().equals(dtype)) {
                    try {
                        lock.lock();
                        log.info("Forcing " + dtype + " to generate flag file");
    ...
    private void initStats(long startTime) {
        ...
        ctx.putIfAbsent(nsa.datawave.metrics.util.flag.InputFile.FLAGMAKER_START_TIME, startTime);
    }
    
    private void updateStats() {
        ctx.getCounter(nsa.datawave.metrics.util.flag.InputFile.FLAGMAKER_END_TIME).setValue(System.currentTimeMillis());
    }
    ...
    public boolean isValidSize(FlagDataTypeConfig fc, Collection<InputFile> files) {
        ...
        if (calculateCounters(files.size()) > maxCounters) {
            log.warn("Check hadoop configuration. Counter limit (" + maxCounters + ") exceeded for " + fc.getDataName() + ". " + "Restricting to "
                            + filesPerPartition(maxCounters) + " files per flag file.");
            return false;
        }
        
        // now check the flag file size
        if (getFlagFileSize(fc, files) > fmc.getMaxFileLength()) {
            log.warn("Flag file size for " + fc.getDataName() + " exceeding " + fmc.getMaxFileLength() + ".  Reducing number of files to compensate");
            return false;
        }
