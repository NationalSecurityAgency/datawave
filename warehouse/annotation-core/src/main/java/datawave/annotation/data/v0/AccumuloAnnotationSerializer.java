...
public class AccumuloAnnotationSerializer implements AnnotationSerializer<Iterator<Map.Entry<Key,Value>>,Annotation> {
    ...
    final AnnotationVisibilityTransformer visibilityTransformer;

    public AccumuloAnnotationSerializer() {
        this(new DefaultAnnotationVisibilityTransformer());
    }

    public AccumuloAnnotationSerializer(AnnotationVisibilityTransformer visibilityTransformer) {
    ...
    public Annotation deserialize(Iterator<Map.Entry<Key,Value>> elements) throws AnnotationSerializationException {
            ...
            if (cqParts.length == 3) {
                ...
                try {
                    ...
                    segments.add(Segment.newBuilder()
                            .setSegmentId(cqParts[1])
                            .setSegmentData(SegmentData.parseFrom(e.getValue().get())).build());
                    //@formatter:on
        ...
        return annotationBuilder.setMetadata(metadata).setSegments(segments).build();
    }
    ...
    protected Key generateBaseKey(Annotation annotation) throws AnnotationSerializationException {
        ...
        ColumnVisibility cv = visibilityTransformer.toColumnVisibility(annotation.getMetadata());

        // TODO: add timestamp?
        return Key.builder().row(rowId).family(columnFamily).visibility(cv).build();
    }
