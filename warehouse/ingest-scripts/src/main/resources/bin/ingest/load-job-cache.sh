...
# fails quietly leaves ingest running against an incomplete or stale cache with nothing to show
# for it. The lock is held via flock on an open descriptor, so it is released when we exit.
function fatal() {
    echo "[ERROR] $*" >&2
    # OLD_JOB_CACHE_DIR is not set until the swap below, so fall back to the one still in effect.
    echo "[ERROR] Job cache was NOT swapped; ingest continues to use ${OLD_JOB_CACHE_DIR:-$JOB_CACHE_DIR}" >&2
    exit 1
}
...
./create-edgekey-version-cache.sh --update ../../config \
    || fatal "Failed to create the edge key version cache; edge ingest would use stale key versions"
...
sed s%${BEFORE}%${AFTER}% job-cache-env.sh > job-cache-env.tmp \
    || fatal "Failed to generate job-cache-env.tmp from job-cache-env.sh"
...
# Capture the status first: without it, "exit $?" reports the status of the rm, so any failure
# below would be reported to the caller as success.
trap 'rc=$?; rm -r -f "$tmpdir"; exit $rc' INT TERM EXIT
...
$INGEST_HADOOP_HOME/bin/hadoop fs -conf $INGEST_HADOOP_CONF/hdfs-site.xml -fs $INGEST_HDFS_NAME_NODE -mkdir -p $INGEST_HDFS_NAME_NODE${JOB_CACHE_DIR}/.. \
    || fatal "Failed to create the ingest job cache parent directory $INGEST_HDFS_NAME_NODE${JOB_CACHE_DIR}/.."
$INGEST_HADOOP_HOME/bin/hadoop fs -conf $INGEST_HADOOP_CONF/hdfs-site.xml -fs $INGEST_HDFS_NAME_NODE -copyFromLocal -t $CPUS ${tmpdir} $INGEST_HDFS_NAME_NODE${JOB_CACHE_DIR} \
    || fatal "Failed to load the ingest job cache into $INGEST_HDFS_NAME_NODE${JOB_CACHE_DIR}"
...
if [[ "$WAREHOUSE_HDFS_NAME_NODE" != "$INGEST_HDFS_NAME_NODE" ]]; then
   ...
   $WAREHOUSE_HADOOP_HOME/bin/hadoop fs -conf $WAREHOUSE_HADOOP_CONF/hdfs-site.xml -fs $WAREHOUSE_HDFS_NAME_NODE -mkdir -p $WAREHOUSE_HDFS_NAME_NODE${JOB_CACHE_DIR}/.. \
       || fatal "Failed to create the warehouse job cache parent directory $WAREHOUSE_HDFS_NAME_NODE${JOB_CACHE_DIR}/.."
   $WAREHOUSE_HADOOP_HOME/bin/hadoop fs -conf $WAREHOUSE_HADOOP_CONF/hdfs-site.xml -fs $WAREHOUSE_HDFS_NAME_NODE -copyFromLocal -t $CPUS ${tmpdir} $WAREHOUSE_HDFS_NAME_NODE${JOB_CACHE_DIR} \
       || fatal "Failed to load the warehouse job cache into $WAREHOUSE_HDFS_NAME_NODE${JOB_CACHE_DIR}"
   # Only do setrep for an hdfs filesystem. Others, such as local or abfs, don't support or need the replication to be set.
...
if [[ -n "${ACTIVE_JOB_CACHE_PATH}" ]]; then
  # Fatal rather than logged and ignored: if Zookeeper still advertises the previous job cache,
  # swapping job-cache-env.sh below would leave the two disagreeing about which cache is active.
  if ! java -cp ${CLASSPATH} datawave.ingest.jobcache.SetActiveCommand \
    ...
    --job-cache "${INGEST_HDFS_NAME_NODE}${JOB_CACHE_DIR}"; then
      fatal "Failed to set active ingest job cache"
  fi
  ...
  if [[ "$WAREHOUSE_HDFS_NAME_NODE" != "$INGEST_HDFS_NAME_NODE" ]]; then
    if ! java -cp ${CLASSPATH} datawave.ingest.jobcache.SetActiveCommand \
      ...
      --job-cache "${WAREHOUSE_HDFS_NAME_NODE}${JOB_CACHE_DIR}"; then
        fatal "Failed to set active warehouse job cache"
    fi
  fi
fi
...
cp job-cache-env.sh job-cache-env.bak || fatal "Failed to back up job-cache-env.sh"
mv job-cache-env.tmp job-cache-env.sh || fatal "Failed to activate the new job-cache-env.sh"
