define([
  ...
  var QueryResultsSummaryView = Backbone.View.extend({
    ...
    render: function() {
      ...
      this.$el.empty();
      var compiledTemplate = _.template(template, { loadingMsg: "Retrieving query metrics..."} );
    ...
    _update: function(collection, interval) {
      ...
      if (collection) {
        collection.each(function(d, i) {
          ...
          coptions.series[0].data.push([parseInt(date), d.get('QueryCount')]);
          
          if (_.isNumber(d.get('QueryCount')) && d.get('QueryCount') > 0) coptions.series[1].data.push([parseInt(date), Math.floor(d.get('TotalPageResultSize') / d.get('QueryCount') )]);
        });
      }
    ...
    chartOptions: {
      ...
      series: [
        ...
        {
          pointInterval: this._interval,
          name: 'Avg Page Results Per Query',
          data: []
        }
      ],
      tooltip: {
  ...
  return QueryResultsSummaryView;
});