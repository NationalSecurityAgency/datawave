...
public abstract class GroupingTest {
    ...
    private void assertResponseEventsAreIdenticalForAllTestResults() {
        ...
        for (QueryResult result : queryResults) {
            ...
            if (prevEvents != null) {
                assertThat(events).describedAs(
                                "Assert events are identical between result from (teardown: %s, interrupt: %s) and (teardown: %s, interrupt: %s)",
                                result.teardown, result.interrupt, prevTeardown, prevInterrupt).isEqualTo(prevEvents);
            }
            prevEvents = events;
