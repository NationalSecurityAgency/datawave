...
/**
  */
public abstract class CompareSQLandDLTest {
    
    @RunWith(Arquillian.class)
    public static class ShardRange extends CompareSQLandDLTest {
    ...
    @RunWith(Arquillian.class)
    public static class DocumentRange extends CompareSQLandDLTest {
        protected static AccumuloClient client = null;
    ...
    private static final Logger log = Logger.getLogger(CompareSQLandDLTest.class);
    ...
    protected ShardQueryLogic shardQueryLogic;
    ...
    public void setup() {
        ...
        documentLogic.setFullTableScanEnabled(true);
        shardQueryLogic.setFullTableScanEnabled(true);
    ...
    protected void runTestQuery(List<String> expected, String querystr, Date startDate, Date endDate, Map<String,String> extraParms, AccumuloClient client)
        ...
        GenericQueryConfiguration dlConfig = documentLogic.initialize(client, settings, authSet);
        documentLogic.setupQuery(dlConfig);
        Set<Document> dlDocs = new HashSet<>();
        for (SerializedDocumentIfc entry : documentLogic) {
            Document d = entry.getAsDocument();
            log.info("dl doc is "+d);
            dlDocs.add(d);
        }

        GenericQueryConfiguration sqlConfig = shardQueryLogic.initialize(client, settings, authSet);
        shardQueryLogic.setupQuery(sqlConfig);
        Set<Document> sqlDocs = new HashSet<>();
        for (Map.Entry<Key, Value> entry : shardQueryLogic) {
            Document d = deserializer.apply(entry).getValue();
            log.info("sql doc is "+d);
            sqlDocs.add(d);
        }
        Assert.assertTrue("mismatch", dlDocs.containsAll(sqlDocs));
    }
