...
public class TestLuceneToJexlQueryParser {
    ...
    public void testRanges() throws ParseException {
        Assert.assertEquals("((BR = true) && (fieldName >= 'aaa' && fieldName <= 'bbb'))", parseQuery("fieldName:[aaa TO bbb]"));
        Assert.assertEquals("((BR = true) && (fieldName > 'aaa' && fieldName < 'bbb'))", parseQuery("fieldName:{aaa TO bbb}"));
        Assert.assertEquals("fieldName1 == 'value1' && ((BR = true) && (fieldName2 >= 'aaa' && fieldName2 <= 'bbb')) && fieldName3 == 'value3'",
                        parseQuery("fieldName1:value1 AND fieldName2:[aaa TO bbb] AND fieldName3:value3"));
        Assert.assertEquals("((BR = true) && (F >= 'A' && F <= 'B'))", parseQuery("F:[A TO B]"));
        Assert.assertEquals("((BR = true) && (F >= 'A\\\\*' && F <= 'B'))", parseQuery("F:[A\\\\\\* TO B]"));
        Assert.assertEquals("((BR = true) && (F > 'lower' && F <= 'upper'))", parseQuery("F:{lower TO upper]"));
        Assert.assertEquals("((BR = true) && (F >= 'lower' && F < 'upper'))", parseQuery("F:[lower TO upper}"));
    }
    ...
    public void testMiscEscapeTokenization() throws ParseException {
        ...
        Assert.assertEquals("((BR = true) && (TOKFIELD >= 'A\\'\\\\*' && TOKFIELD <= 'B\\''))", parseQuery("TOKFIELD:[A'\\\\\\* TO B']"));
    ...
    public void testNumericRange() throws ParseException {
        Assert.assertEquals("((BR = true) && (FOO >= '1' && FOO <= '5000'))", parser.parse("FOO:[1 TO 5000]").getOriginalQuery());
    }
