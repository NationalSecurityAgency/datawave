...
public class IpAddressQueryTest extends AbstractFunctionalQuery {
    ...
    public void testRange() throws Exception {
        String query = "((BR = true) && (" + IpAddrField.PUBLIC_IP.name() + GTE_OP + "'9.9.9.9'" + AND_OP + IpAddrField.PUBLIC_IP.name() + LTE_OP
                        + "'9.9.40.1'))";
        runTest(query, query);
    }
    ...
    public void testRangeWithNotEq() throws Exception {
        String query = "((BR = true) && (" + IpAddrField.PRIVATE_IP.name() + GTE_OP + "'20.20.20.20'" + AND_OP + IpAddrField.PRIVATE_IP.name() + LTE_OP
    ...
    public void testMultiRange() throws Exception {
        String query = "((BR = true) && (" + IpAddrField.PRIVATE_IP.name() + GTE_OP + "'20.20.20.20'" + AND_OP + IpAddrField.PRIVATE_IP.name() + LTE_OP
                        + "'22.90.90.200')" + ")" + OR_OP + "((((BR = true) && (" + IpAddrField.PRIVATE_IP.name() + GTE_OP + "'33.60.60.60'" + ")" + AND_OP
                        + IpAddrField.PRIVATE_IP.name() + LTE_OP + "'33.100.100.200'" + ")))";
        runTest(query, query);
    }
