...
public class FixUnfieldedTermsVisitorTest {
    ...
    private static InMemoryInstance instance = new InMemoryInstance(FixUnfieldedTermsVisitor.class.toString());
    ...
    public void testSingleTermMatchesWithExpansionFields() {
        try {
            ...
            Set<String> expansionFields = ImmutableSet.of("FIELD1", "FIELD2");
            ASTJexlScript fixed = FixUnfieldedTermsVisitor.fixUnfieldedTree(config, scannerFactory, metadataHelper, script, expansionFields, true, true, true);
    ...
    public void testSingleTermMatchesWithMisMatchedExpansionFields() {
        try {
            ...
            ASTJexlScript script = JexlASTHelper.parseJexlQuery(query);
            ScannerFactory scannerFactory = new ScannerFactory(config.getConnector());
            Set<String> expansionFields = ImmutableSet.of("FOOBAR");
            ASTJexlScript fixed = FixUnfieldedTermsVisitor.fixUnfieldedTree(config, scannerFactory, metadataHelper, script, expansionFields, true, true, true);
    ...
    public void testNegatedLeadingRegexExpansionRestrictedByExpansionFields() {
        try {
            ...
            ASTJexlScript script = JexlASTHelper.parseJexlQuery(query);
            ScannerFactory scannerFactory = new ScannerFactory(config.getConnector());
            Set<String> expansionFields = ImmutableSet.of("FIELD8", "FIELD9");
            ASTJexlScript fixed = FixUnfieldedTermsVisitor.fixUnfieldedTree(config, scannerFactory, metadataHelper, script, expansionFields, true, true, true);
    ...
    @Test(expected = CannotExpandUnfieldedTermFatalException.class)
    ...
    public void test(String query, String expected, ShardQueryConfiguration config, MockMetadataHelper metadataHelper) throws Exception {
        ASTJexlScript script = JexlASTHelper.parseJexlQuery(query);
        ScannerFactory scannerFactory = new ScannerFactory(config.getConnector());
        ASTJexlScript fixed = FixUnfieldedTermsVisitor.fixUnfieldedTree(config, scannerFactory, metadataHelper, script, true, true, true);
