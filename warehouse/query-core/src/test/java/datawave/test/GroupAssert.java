...
public class GroupAssert extends AbstractAssert<GroupAssert,Group> {
    ...
    public GroupAssert hasVisibilitiesForKey(GroupingAttribute<?> key, ColumnVisibility... visibilities) {
        ...
        Assertions.assertThat(actualVisibilities)
                        .isNotNull()
                        .withFailMessage("Expected column visibilties for %s to contain exactly %s but was %s", key, Arrays.toString(visibilities),
                                        actualVisibilities).containsExactlyInAnyOrder(visibilities);
        return this;
    }
