...
public class RegexDocIdIterator extends DocIdIterator {
    ...
    protected Range createScanRange() {
        try {
            ...
            if (analyzer.isLeadingLiteral()) {
                ...
                Key stop = new Key(row, prefix, literal + '\uffff');
                ...
                Key stop = new Key(row, prefix, Constants.MAX_UNICODE_STRING);
                return new Range(start, true, stop, false);
            }
    ...
    /**
     * If a datatype filter is set the iterator can seek to the next available datatype, or rollover to the next value
     */
