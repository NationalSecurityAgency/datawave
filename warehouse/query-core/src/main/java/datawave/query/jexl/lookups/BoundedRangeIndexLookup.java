...
public class BoundedRangeIndexLookup extends IndexLookup {
    ...
    protected long maxLookup;
    protected ScannerFactory scannerFactory;
    protected BatchScanner bs;
    protected IndexLookupMap fieldToUniqueTerms;
    
    boolean setupComplete = false;
    boolean lookupComplete = false;
    
    public BoundedRangeIndexLookup(LiteralRange<?> literalRange) {
        this.literalRange = literalRange;
        datatypeFilter = Sets.newHashSet();
    }
    
    @Override
    public synchronized void lookupAsync(ShardQueryConfiguration config, ScannerFactory scannerFactory, long maxLookup, ExecutorService execService) {
        if (!setupComplete) {
            setupComplete = true;
            ...
            fieldToUniqueTerms = new IndexLookupMap(config.getMaxUnfieldedExpansionThreshold(), config.getMaxValueExpansionThreshold());
            ...
            try {
                ...
                timedScanAsync(bs.iterator(), fieldToUniqueTerms, config, false, fields, false, maxLookup, null, execService);
