...
public class EvaluationPhaseFilterFunctions {
    ...
    /**
     * We don't have access, at this layer, to the type information. We do have the typed value, so we must expect
     * there to be type T that matches this. If they do not the comparison will result in a class cast exception, through
     * which we we can attempt to create the corresponding hard type.
     * @param fieldValue ValueTuple that contains the raw field value
     * @param lowerBound lower bound, expected to be the same type as the raw field value
     * @param upperBound upper bound, expected to be the same field value.
     * @param <T> Generic type of the bounds arguments.
     * @return
     */
