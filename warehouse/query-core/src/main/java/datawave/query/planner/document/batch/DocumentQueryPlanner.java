...
import datawave.webservice.query.exception.*;
...
public class DocumentQueryPlanner extends QueryPlanner implements Cloneable {

    private static final Logger log = ThreadConfigurableLogger.getLogger(DocumentQueryPlanner.class);
    ...
    protected ASTJexlScript updateQueryTree(ScannerFactory scannerFactory, MetadataHelper metadataHelper, DateIndexHelper dateIndexHelper,
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Determine if query contains composite fields");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Determine if we can deal with unsorted UIDs");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Determine if query contains term frequency (tokenized) fields");
        ...
        if (reduceQuery) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - final reduce query");
    ...
    protected ASTJexlScript processTree(final ASTJexlScript originalQueryTree, ShardQueryConfiguration config, Query settings, MetadataHelper metadataHelper,
        ...
        if (!disableBoundedLookup) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand bounded query ranges (total)");
    ...
    @Override
    public DocumentQueryPlanner clone() {
        return new DocumentQueryPlanner(this);
    }
