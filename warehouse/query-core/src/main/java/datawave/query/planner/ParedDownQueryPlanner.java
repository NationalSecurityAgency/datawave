...
public class ParedDownQueryPlanner extends QueryPlanner implements Cloneable {

    private static final Logger log = ThreadConfigurableLogger.getLogger(ParedDownQueryPlanner.class);
    ...
    public ParedDownQueryPlanner() {
        this(Long.MAX_VALUE);
    }

    public ParedDownQueryPlanner(long _maxRangesPerQueryPiece) {
        this(_maxRangesPerQueryPiece, true);
    }

    public ParedDownQueryPlanner(long maxRangesPerQueryPiece, boolean limitScanners) {
        this.maxRangesPerQueryPiece = maxRangesPerQueryPiece;
        setLimitScanners(limitScanners);
    }

    protected ParedDownQueryPlanner(ParedDownQueryPlanner other) {
    ...
    protected CloseableIterable<QueryData> process(ScannerFactory scannerFactory, MetadataHelper metadataHelper, DateIndexHelper dateIndexHelper,
        ...
        try {
            ...
            return DocumentQueryPlanner.emptyCloseableIterator();
        }
        ...
        TraceStopwatch stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Rebuild JEXL String from AST");
        ...
        stopwatch.stop();
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Construct IteratorSettings");
    ...
    public void close(GenericQueryConfiguration genericConfig, Query settings) {
        if (!(genericConfig instanceof ShardQueryConfiguration)) {
            if (genericConfig != null) {
                log.warn("Config object must be an instance of ShardQueryConfiguration to properly close the DocumentQueryPlanner. You gave me a "
                        + genericConfig);
            }
    ...
    public static void validateQuerySize(String lastOperation, JexlNode queryTree, ShardQueryConfiguration config, boolean timed) {
        ...
        if (timed) {
            final QueryStopwatch timers = config.getTimers();
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Validate against term and depth thresholds");
        }
    ...
    protected ASTJexlScript updateQueryTree(ScannerFactory scannerFactory, MetadataHelper metadataHelper, DateIndexHelper dateIndexHelper,
        ...
        TraceStopwatch stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Parse query");
...
/*
        if (query.contains(QueryFunctions.QUERY_FUNCTION_NAMESPACE + ':')) {
            ...
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - parse out queryOptions from options function");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - fix not null intent");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - include date filters");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - cap date range");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - find unmarked unbounded ranges");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - flatten");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - initial re-write");
        ...
        stopwatch.stop();
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Restructure negative numbers");
        ...
        stopwatch.stop();
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Uppercase all field names");
        ...
        stopwatch.stop();
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Rewrite negated equality operators.");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Apply query model");
        ...
        if (config.getEnforceUniqueTermsWithinExpressions()) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Enforce unique terms within AND and OR expressions");
        ...
        if (config.getEnforceUniqueConjunctionsWithinExpression()) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Enforce unique AND'd terms within OR expressions");
        ...
        if (config.getEnforceUniqueDisjunctionsWithinExpression()) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Enforce unique OR'd terms within AND expressions");
...
*/
        ...
        queryTree = processTree(queryTree, config, settings, metadataHelper, scannerFactory, queryData, timers, null);

        // ExpandCompositeTerms was here

        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Determine if query contains index-only fields");
        ...
        config.setContainsIndexOnlyTerms(true);
...
/*
        if (!disableExpandIndexFunction) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand function index queries");
...
/*
        if (!disableAnyFieldLookup) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand ANYFIELD regex nodes");
...
*/
        if (reduceQuery) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - reduce query after anyfield expansions");
...
/*
        if (!disableTestNonExistentFields) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Test for non-existent fields");
...
*/
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Fetch required dataTypes");
        ...
        stopwatch.stop();
        /*
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Fix unindexed numerics");
        ...
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand query from normalizers");
...
/*
        ...
        if (!config.getIndexHoles().isEmpty()) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Mark Index Holes");
        ...
        if (executableExpansion) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Executable expansion");
...
*/
       ...
       // queryTree = applyNodeTransformRules(queryTree, getTransformRules(), config, metadataHelper, "Pre pushdown-pullup");
        ...
        if (config.getMinSelectivity() > 0) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Pushdown Low-Selective Terms");
...
/*
        stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Force field-to-field comparison to evaluation only");
...
/*
        if (!disableCompositeFields) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand composite terms");
...
*/
        if (!disableBoundedLookup) {
            stopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand bounded query ranges (total)",true);
            TraceStopwatch innerStopwatch = null;

            // Expand any bounded ranges into a conjunction of discrete terms
            try {
                innerStopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand regex");
                ...
                innerStopwatch.stop();
/*
                innerStopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand ranges");
                ...
                if (reduceQuery) {
                    innerStopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - reduce after range expansion");
...
*/
                ...
                /*
                innerStopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Prune GeoWave terms");
                ...
                innerStopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Expand pushing functions into exceeded value ranges");
                ...
                innerStopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Remove delayed predicates");
                ...
                // possible
                /*
                innerStopwatch = timers.newStartedStopwatch("DocumentQueryPlanner - Add delayed predicates");
                if (log.isDebugEnabled()) {
                    debugOutput.clear();
                }
                if (!ExecutableDeterminationVisitor
    ...
    public ASTJexlScript applyNodeTransformRules(ASTJexlScript queryTree, List<NodeTransformRule> rules, ShardQueryConfiguration config, MetadataHelper helper,
        ...
        if (!rules.isEmpty()) {
            final TraceStopwatch stopwatch = config.getTimers().newStartedStopwatch("DocumentQueryPlanner - Apply Node Transform Rules: " + instance);
                ...
                try {
                    ...
                    if (compositeMetadata != null && !compositeMetadata.isEmpty())
                        addOption(cfg, QueryOptions.COMPOSITE_METADATA,
                                Base64.getEncoder().encodeToString(CompositeMetadata.toBytes(compositeMetadata)), false);
                } catch (TableNotFoundException e) {
    ...
    public Tuple2<CloseableIterable<QueryPlan>,Boolean> getQueryRanges(ScannerFactory scannerFactory, MetadataHelper metadataHelper,
        ...
        if (!needsFullTable) {
            ...
            TraceStopwatch stopwatch = config.getTimers().newStartedStopwatch("DocumentQueryPlanner - Begin stream of ranges from inverted index");
    ...
    @Override
    public ParedDownQueryPlanner clone() {
        return new ParedDownQueryPlanner(this);
    }
    ...
    public static void setMaxChildNodesToPrint(int maxChildNodesToPrint) {
        DocumentQueryPlanner.maxChildNodesToPrint = maxChildNodesToPrint;
    }
