 ...
 *
 * This class and its methods aren't static so that we don't run into concurrency issues, although all required state should
 * be passed into the individual methods and not kept in this class. Calling classes could extend this class to inherit
 * the methods, but the state still shouldn't be inherited because not all callers will be able to easily extend this
 * class if they already/need to extend other parents.
 */
public class GroupingUtil {
    ...
    public GroupingInfo getGroupingInfo(Map.Entry<Key, Document> entry, MarkingFunctions markingFunctions, Set<String> groupFieldsSet) {
        return getGroupingInfo(entry, markingFunctions, groupFieldsSet, null);
    }

    public GroupingInfo getGroupingInfo(Map.Entry<Key, Document> entry, MarkingFunctions markingFunctions, Set<String> groupFieldsSet, Map<String,String> reverseModelMapping) {
        ...
        // and equals methods
        GroupCountingHashMap countingMap = new GroupCountingHashMap(markingFunctions);
        ...
        if (entry != null) {
            ...
            Multimap<String, String> fieldToFieldWithContextMap = getFieldToFieldWithGroupingContextMap(entry.getValue(),
                    expandedGroupFieldsList, fieldMap, groupFieldsSet, reverseModelMapping);
    ...
    private Multimap<String,String> getFieldToFieldWithGroupingContextMap(Document d, Set<String> expandedGroupFieldsList,
                                                                          Map<String, GroupingTypeAttribute<?>> fieldMap,
                                                                          Set<String> groupFieldsSet, Map<String,String> reverseModelMapping) {
