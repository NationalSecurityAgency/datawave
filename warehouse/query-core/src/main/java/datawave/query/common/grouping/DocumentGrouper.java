...
public class DocumentGrouper {
    ...
    private final TreeMultimap<String,TemporalGranularity> groupFields;
    ...
    private DocumentGrouper(Map.Entry<Key,Document> documentEntry, GroupFields groupFields, Groups groups) {
        ...
        this.groupFields = groupFields.getGroupByFieldMap();
    ...
    private void group() {
        if (log.isDebugEnabled()) {
            log.debug("Applying grouping {} to {} {}", groupFields, documentKey, document);
        }
        ...
        if (isDocumentAlreadyGrouped()) {
            if (log.isTraceEnabled()) {
                log.trace("Groups from previous grouping found in document {}", documentKey);
            }
            extractGroupsFromDocument();
        } else {
            // Otherwise, the document contains entries that have not yet been grouped and counted.
            if (log.isTraceEnabled()) {
                log.trace("No previous groupings in document {}", documentKey);
            }
            ...
            if (fieldAggregatorFactory.hasFieldsToAggregate() && !aggregateFieldsIndex.isEmpty()) {
                if (log.isTraceEnabled()) {
                    log.trace("Fields found for aggregation in document {}", documentKey);
                }
                aggregateEntries();
            } else {
    ...
    private void extractGroupsFromDocument() {
        if (log.isTraceEnabled()) {
            log.trace("Extracting existing groups from document {}", documentKey);
        }
        ...
        for (String instance : idToFields.keySet()) {
            ...
            if (log.isTraceEnabled()) {
                log.trace("Extracted grouping {} with count of {} from document {}", grouping, count, documentKey);
            }
    ...
    private void groupEntries() {
        if (log.isTraceEnabled()) {
            log.trace("Grouping entries for document {}", documentKey);
        }

        // If we found any entries for target group fields after indexing, identify all valid groupings.
        if (!groupFieldsIndex.isEmpty()) {
            ...
            if (maxGroupSize == 1) {
                log.trace("Max group size is 1, only singleton groupings required.");
                groupFieldsIndex.fields.values().stream().map(Collections::singleton).forEach(groupings::add);
            } else {
                if (log.isTraceEnabled()) {
                    log.trace("Max group size is {}, attempting to find matches", maxGroupSize);
                }
                ...
                if (groupFieldsIndex.hasFieldsWithPossibleDirectMatch()) {
                    log.trace("Possible direct matches.");
                    groupings = getGroupingsWithDirectMatches();
                } else {
                    log.trace("No possible direct matches.");
                }
                ...
                // should be combined with each existing group combination, effectively creating cartesian products.
                if (groupFieldsIndex.hasFieldsWithoutDirectMatch()) {
                    log.trace("Possible indirect matches.");
                    groupings = getGroupingsWithoutDirectMatches(groupings);
                } else {
                    log.trace("No possible indirect matches.");
                }
            }
            ...
            // grouping results.
            if (log.isTraceEnabled()) {
                log.trace("No valid groupings found in group fields index, using empty grouping for document {}", documentKey);
            }
    ...
    private List<Set<Field>> getGroupingsWithDirectMatches() {
        if (log.isTraceEnabled()) {
            log.trace("Searching for groupings with direct matches for document {}", documentKey);
        }
        List<Set<Field>> groupings = new ArrayList<>();
        ...
        if (fieldsWithGroupingContextAndInstance.size() == 1) {
            String field = fieldsWithGroupingContextAndInstance.iterator().next();
            if (log.isTraceEnabled()) {
                log.trace("Only one field found with a grouping context and instance: {}. Returning singleton groupings for each distinct value.", field);
            }
            Collection<Field> fields = groupFieldsIndex.getFields(field);
            fields.stream().map(Collections::singleton).forEach(groupings::add);
        } else {
            log.trace("Multiple fields found with grouping contexts and instances.");
            ...
            for (Map.Entry<Pair<String,String>,Collection<Field>> entry : directMatchesSortedByPrevalence) {
                ...
                for (Field field : fields) {
                    ...
                    if (fieldToLargestGroupingSize.containsKey(field.getBase())) {
                        ...
                        if (log.isTraceEnabled()) {
                            log.trace("Establishing largest batch size for field {} is {}", field.getBase(), fields.size());
                        }
                        keep = true;
                    }
                }
                if (keep) {
                    if (log.isTraceEnabled()) {
                        log.trace("Retaining entries as direct matches for fields {}", groupingFields);
                    }
            ...
            log.trace("Creating cartesian products between direct matches.");
    ...
    private List<Set<Field>> getGroupingsWithoutDirectMatches(List<Set<Field>> prevGroupings) {
        if (log.isTraceEnabled()) {
            log.trace("Searching for groupings without direct matches for document {}", documentKey);
        }
    ...
    private void trackGroup(Collection<Field> groupedFields) {
        if (log.isTraceEnabled()) {
            log.trace("Tracking group {} for document {}", groupedFields, documentKey);
        }
        ...
        for (Field field : groupedFields) {
            ...
            // Create the set of distinct grouping attributes for each value of the field in the event.
            Set<GroupingAttribute<?>> groupingAttributes = createGroupingAttributes(field.getBase(), field.getAttributes());

            // If we have no grouping keys yet, create initial groupings that consist just of each distinct field value.
            if (groupings.isEmpty()) {
                for (GroupingAttribute<?> attribute : groupingAttributes) {
                    groupings.add(new Grouping(attribute));
                }
            } else {
                // Otherwise, create the cartesian product between the current field's value and each existing grouping.
                List<Grouping> newGroupings = new ArrayList<>();
                for (GroupingAttribute<?> attribute : groupingAttributes) {
                    for (Grouping grouping : groupings) {
                        Grouping groupingCopy = new Grouping(grouping);
                        groupingCopy.add(attribute);
    ...
    private void trackGroup(Grouping grouping) {
        ...
        if (group == null) {
            if (log.isTraceEnabled()) {
                log.trace("Creating new Group for grouping {}", grouping);
            }
    ...
    private Set<GroupingAttribute<?>> createGroupingAttributes(String field, Set<Attribute<?>> attributes) {
        ...
        // Create grouping attributes for each value of the field.
        for (Attribute<?> attribute : attributes) {
            Type<?> type = ((TypeAttribute<?>) attribute).getType();
            // If no temporal granularities were specified, add a grouping attribute of the original value.
            if (granularities.isEmpty()) {
                GroupingAttribute<?> newAttribute = new GroupingAttribute<>(type, new Key(field), true);
                newAttribute.setColumnVisibility(attribute.getColumnVisibility());
                groupingAttributes.add(newAttribute);
            } else {
                // Otherwise, create a grouping attribute for each transformed version of the value resulting from applying each temporal granularity to it.
                for (TemporalGranularity granularity : granularities) {
                    if (granularity == TemporalGranularity.ALL) {
                        // Do not transform the value for the granularity ALL.
                        GroupingAttribute<?> newAttribute = new GroupingAttribute<>(type, new Key(field), true);
                        newAttribute.setColumnVisibility(attribute.getColumnVisibility());
                        groupingAttributes.add(newAttribute);
                    } else {
    ...
    private void aggregateEntries() {
        if (log.isTraceEnabled()) {
            log.trace("Aggregating entries for document {}", documentKey);
        }
        ...
        if (!groupFieldsIndex.isEmpty()) {
            if (log.isTraceEnabled()) {
                log.trace("Groupings found for document {}. Fields will be aggregated to these groupings.", documentKey);
            }
            ...
            if (aggregateFieldsIndex.hasFieldsWithPossibleDirectMatch()) {
                log.trace("Aggregating fields with possible direct matches.");
            ...
            if (aggregateFieldsIndex.hasFieldsWithoutDirectMatch()) {
                log.trace("Aggregating fields without direct match.");
            ...
            if (log.isTraceEnabled()) {
                log.trace("No groupings found for document {}. Fields will be aggregated to an empty grouping.", documentKey);
            }
