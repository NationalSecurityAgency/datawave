 ...
 *
 */
public class SortedByteMapBuffer extends AbstractSet<byte[]> implements SortedSet<byte[]> {
    ...
    public SortedByteMapBuffer() {
        this(DEFAULT_BUFFER_SIZE);
    }

    public SortedByteMapBuffer(int capacity) {
    ...
    protected class SortedByteSetBufferIterator implements Iterator<byte[]> {
        ...
        public void remove() {
            checkModCount();
            if (last >= 0) {
                SortedByteMapBuffer.this.remove(last);
                index--;
                end--;
    ...
    protected class SortedByteSubSetBuffer extends AbstractSet<byte[]> implements SortedSet<byte[]> {
        ...
        @Override
        public Comparator<? super byte[]> comparator() {
            return SortedByteMapBuffer.this.comparator();
        }
        ...
        public SortedSet<byte[]> subSet(byte[] fromElement, byte[] toElement) {
            ...
            return SortedByteMapBuffer.this.subSet(fromElement, toElement);
        }
        ...
        public boolean add(byte[] e) {
            ...
            return SortedByteMapBuffer.this.add(e);
        }

        @Override
        public boolean remove(Object o) {
            if (contains(o)) {
                return SortedByteMapBuffer.this.remove(o);
            }
            return false;
        }
        ...
        protected int[] getRange() {
            if (expectedModCount != modCount) {
                expectedModCount = modCount;
                if (SortedByteMapBuffer.this.isEmpty()) {
                    range = null;
                } else {
                    ...
                    if (start == SortedByteMapBuffer.this.size()) {
                        range = null;
                    } else {

                        // find the last entry
                        int end = (to == null ? SortedByteMapBuffer.this.size() : binarySearch(to));
                        if (end < 0) {
                            end = -1 - end;
                        }
