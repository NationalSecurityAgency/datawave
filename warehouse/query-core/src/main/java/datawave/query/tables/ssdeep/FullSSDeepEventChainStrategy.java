...
public class FullSSDeepEventChainStrategy extends FullChainStrategy<ScoredSSDeepPair,Entry<Key, Value>> {
    ...
    /**
     * Given a single discovered event field, check to see if it's a matched SSDeep entry. If so, look up the hash
     * in the scoredMatches map and generate additional Entries that capture the information from the ScoredSSDeepPair.
     * Otherwise, pass through the event unchanged.
     *
     * @param eventSSDeep
