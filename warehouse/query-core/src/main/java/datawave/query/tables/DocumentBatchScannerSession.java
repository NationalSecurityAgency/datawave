...
public class DocumentBatchScannerSession extends DocumentScannerSession implements Iterator<SerializedDocumentIfc>, FutureCallback<DocumentScanner>, SessionArbiter, UncaughtExceptionHandler {
    ...
    private static final Logger log = Logger.getLogger(DocumentBatchScannerSession.class);
    ...
    protected boolean SpeculativeDocumentScanning = false;
     ...
     * @param maxResults
     */
    public DocumentBatchScannerSession(DocumentQueryConfiguration config, String tableName, Set<Authorizations> auths, ResourceQueue delegator, int maxResults, Query settings, ScannerOptions options,
    ...
    public DocumentBatchScannerSession updateThreadService(ExecutorService service) {
        if (service != null)
    ...
    public DocumentBatchScannerSession setThreads(int threads) {
        if (service != null)
            service.shutdownNow();
    ...
    public DocumentBatchScannerSession updateIdentifier(String threadId) {
     ...
     * 
     */
    public synchronized DocumentBatchScannerSession setChunkIter(Iterator<List<ScannerChunk>> chunkIter) {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof DocumentBatchScannerSession) {
            EqualsBuilder builder = new EqualsBuilder();
            builder.append(localTableName, ((DocumentBatchScannerSession) obj).localTableName);
            builder.append(localAuths, ((DocumentBatchScannerSession) obj).localAuths);
    ...
    protected void submitTasks(List<ScannerChunk> newChunks) {
        ...
        for (ScannerChunk chunk : newChunks) {
            ...
            DocumentScanner scan = null;
            
            if (SpeculativeDocumentScanning) {
                ...
                scan = new SpeculativeDocumentScan(config, localTableName, localAuths, chunk, delegatorReference, (Class<? extends DocumentResource> )delegatedResourceInitializer, resultQueue, listenerService);
                ...
                childScan = new DocumentScanner(config, localTableName, localAuths, new ScannerChunk(chunk), delegatorReference,  (Class<? extends DocumentResource> )delegatedResourceInitializer,
                                ((SpeculativeDocumentScan) scan).getQueue(), listenerService);
                ...
                scan = new DocumentScanner(config, localTableName, localAuths, chunk, delegatorReference,  (Class<? extends DocumentResource> )delegatedResourceInitializer, resultQueue, listenerService);
            }
        ...
        for (ScannerChunk chunk : newChunks) {
            ...
            if (SpeculativeDocumentScanning) {
                ...
                ((SpeculativeDocumentScan) scan).addScan(new DocumentScanner(config, localTableName, localAuths, new ScannerChunk(chunk), delegatorReference,
                        (Class<? extends DocumentResource> )delegatedResourceInitializer, ((SpeculativeDocumentScan) scan).getQueue(), listenerService));
    ...
    protected void submitScan(DocumentScanner scan, boolean increment) {
     ...
     * @param options
     */
    public DocumentBatchScannerSession setOptions(SessionOptions options) {
        return this;
     ...
     */
    @Override
    public void onSuccess(DocumentScanner finishedScan) {
        /**
    ...
    private class BatchScannerListener extends Listener {
    ...
    public void setSpeculativeDocumentScanning(boolean speculative) {
        this.SpeculativeDocumentScanning = speculative;
    }
