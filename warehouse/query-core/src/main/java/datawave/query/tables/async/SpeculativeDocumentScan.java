 ...
 *
 */
public class SpeculativeDocumentScan extends DocumentScanner implements FutureCallback<DocumentScanner>, UncaughtExceptionHandler {
    private static final Logger log = Logger.getLogger(SpeculativeDocumentScan.class);

    protected AtomicInteger successCount = new AtomicInteger(0);

    protected AtomicInteger failureCount = new AtomicInteger(0);

    protected List<DocumentScanner> scans;

    protected List<Future<DocumentScanner>> scanFutures;
    ...
    public boolean addScan(DocumentScanner scan) {
    ...
    @Override
    public void onSuccess(DocumentScanner result) {
        if (log.isTraceEnabled()) {
    ...
    protected void closeScans() {
        for (DocumentScanner scan : scans) {
            scan.close();
        }
