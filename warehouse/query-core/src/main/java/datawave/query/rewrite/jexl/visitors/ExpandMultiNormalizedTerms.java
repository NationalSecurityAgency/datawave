        ...
        if (!lowerBounds.isEmpty() && !upperBounds.isEmpty()) {
            ...
            for (String field : tightBounds) {
                ...
                for (Type<?> normalizer : config.getQueryFieldsDatatypes().get(field)) {
                    ...
                    JexlNode left = null;
                    try {
                        left = JexlASTHelper.applyNormalization(RebuildingVisitor.copy(lowerBound), normalizer);
                    } catch (Exception ne) {
                    ...
                    JexlNode right = null;
                    try {
                        right = JexlASTHelper.applyNormalization(RebuildingVisitor.copy(upperBound), normalizer);
