...
import datawave.query.rewrite.exceptions.CannotExpandUnfieldedTermFatalException;
...
import datawave.query.rewrite.exceptions.DoNotPerformOptimizedQueryException;
import datawave.query.rewrite.exceptions.FullTableScansDisallowedException;
import datawave.query.rewrite.exceptions.InvalidQueryException;
import datawave.query.rewrite.exceptions.NoResultsException;
import datawave.query.rewrite.iterator.CloseableListIterable;
...
import datawave.query.rewrite.jexl.functions.EvaluationPhaseFilterFunctions;
import datawave.query.rewrite.jexl.visitors.BoundedRangeDetectionVisitor;
import datawave.query.rewrite.jexl.visitors.CaseSensitivityVisitor;
...
import datawave.query.rewrite.jexl.visitors.ExpandCompositeTerms;
import datawave.query.rewrite.jexl.visitors.ExpandMultiNormalizedTerms;
import datawave.query.rewrite.jexl.visitors.FetchDataTypesVisitor;
import datawave.query.rewrite.jexl.visitors.FieldMissingFromSchemaVisitor;
import datawave.query.rewrite.jexl.visitors.FixNegativeNumbersVisitor;
import datawave.query.rewrite.jexl.visitors.FixUnfieldedTermsVisitor;
import datawave.query.rewrite.jexl.visitors.FixUnindexedNumericTerms;
import datawave.query.rewrite.jexl.visitors.FunctionIndexQueryExpansionVisitor;
import datawave.query.rewrite.jexl.visitors.IsNotNullIntentVisitor;
import datawave.query.rewrite.jexl.visitors.JexlStringBuildingVisitor;
import datawave.query.rewrite.jexl.visitors.ParallelIndexExpansion;
...
import datawave.query.rewrite.jexl.visitors.PushFunctionsIntoExceededValueRanges;
import datawave.query.rewrite.jexl.visitors.PushdownLowSelectivityNodesVisitor;
import datawave.query.rewrite.jexl.visitors.PushdownMissingIndexRangeNodesVisitor;
...
import datawave.query.rewrite.jexl.visitors.RangeConjunctionRebuildingVisitor;
import datawave.query.rewrite.jexl.visitors.RegexFunctionVisitor;
import datawave.query.rewrite.jexl.visitors.RewriteNegations;
import datawave.query.rewrite.jexl.visitors.SetMembershipVisitor;
import datawave.query.rewrite.jexl.visitors.SortedUIDsRequiredVisitor;
import datawave.query.rewrite.jexl.visitors.TermCountingVisitor;
import datawave.query.rewrite.jexl.visitors.TreeFlatteningRebuildingVisitor;
import datawave.query.rewrite.jexl.visitors.ValidPatternVisitor;
import datawave.query.rewrite.planner.pushdown.PushDownVisitor;
import datawave.query.rewrite.planner.pushdown.rules.PushDownRule;
import datawave.query.rewrite.postprocessing.tf.Function;
import datawave.query.rewrite.postprocessing.tf.TermOffsetPopulator;
...
public class DefaultQueryPlanner extends QueryPlanner {
    ...
    private MetadataHelper getMetadataHelper(final RefactoredShardQueryConfiguration config) {
    ...
    private DateIndexHelper getDateIndexHelper(final RefactoredShardQueryConfiguration config) {
    ...
    public CloseableIterable<QueryData> process(GenericQueryConfiguration genericConfig, String query, Query settings, ScannerFactory scannerFactory)
        ...
        if (!(genericConfig instanceof RefactoredShardQueryConfiguration)) {
            throw new ClassCastException("Config object must be an instance of RefactoredShardQueryConfiguration");
        }
    ...
    private void configureIterator(RefactoredShardQueryConfiguration config, IteratorSetting cfg, String newQueryString, boolean isFullTable)
                    throws DatawaveQueryException {
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.QueryPlanner#close(datawave. webservice .query.configuration.GenericQueryConfiguration,
     * datawave.webservice.query.Query)
     */
    @Override
    ...
    protected ASTJexlScript updateQueryTree(ScannerFactory scannerFactory, MetadataHelper metadataHelper, DateIndexHelper dateIndexHelper,
        ...
        RewriteNegations.rewrite(queryTree);
        if (log.isDebugEnabled()) {
    ...
    protected ASTJexlScript processTree(final ASTJexlScript originalQueryTree, RefactoredShardQueryConfiguration config, Query settings,
                    MetadataHelper metadataHelper, ScannerFactory scannerFactory, QueryData queryData, QueryStopwatch timers, QueryModel queryModel)
                    throws DatawaveQueryException {
        ...
        try {
            if (null != fieldToDatatypeMap) {
                ...
                if (null == queryFieldsAsDataTypeMap) {
                    queryFieldsAsDataTypeMap = HashMultimap.create(Multimaps.filterKeys(fieldToDatatypeMap, new Predicate<String>() {
                        
                        @Override
                        public boolean apply(String input) {
                            return !cachedNormalizedFields.contains(input);
                        }
                    }));
                }
                
                if (null == normalizedFieldAsDataTypeMap) {
                    normalizedFieldAsDataTypeMap = HashMultimap.create(Multimaps.filterKeys(fieldToDatatypeMap, new Predicate<String>() {
    ...
    private void loadDataTypeMetadata(Multimap<String,Type<?>> fieldToDatatypeMap, Set<String> indexedFields, Set<String> normalizedFields,
                    RefactoredShardQueryConfiguration config, boolean reload) throws AccumuloException, AccumuloSecurityException, TableNotFoundException,
    ...
    protected ASTJexlScript applyQueryModel(MetadataHelper metadataHelper, RefactoredShardQueryConfiguration config, TraceStopwatch stopwatch,
                    ASTJexlScript queryTree, QueryModel queryModel) {
    ...
    public ASTJexlScript addDateFilters(ASTJexlScript queryTree, ScannerFactory scannerFactory, MetadataHelper metadataHelper, DateIndexHelper dateIndexHelper,
                    RefactoredShardQueryConfiguration config, Query settings) throws TableNotFoundException, DatawaveQueryException {
     ...
     * @param config
     */
    protected void capDateRange(RefactoredShardQueryConfiguration config) throws DatawaveQueryException {
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.pushdown.PushDownPlanner#rewriteQuery( org.apache .commons.jexl2.parser.ASTJexlScript)
     */
    @Override
    public ASTJexlScript applyRules(ASTJexlScript queryTree, ScannerFactory scannerFactory, MetadataHelper metadataHelper,
                    RefactoredShardQueryConfiguration config) {
    ...
    protected Future<IteratorSetting> loadQueryIterator(final MetadataHelper metadataHelper, final RefactoredShardQueryConfiguration config,
        ...
        return builderThread.submit(new Callable<IteratorSetting>() {
            
            @Override
            public IteratorSetting call() throws Exception {
                ...
                try {
                    ...
                    addOption(cfg, QueryOptions.COMPOSITE_FIELDS,
                                    QueryOptions.buildIndexOnlyFieldsString(metadataHelper.getCompositeToFieldMap(config.getDatatypeFilter()).keySet()), true);
                } catch (TableNotFoundException e) {
                    QueryException qe = new QueryException(DatawaveErrorCode.INDEX_ONLY_FIELDS_RETRIEVAL_ERROR, e);
                    throw new DatawaveQueryException(qe);
                }
    ...
    protected IteratorSetting getQueryIterator(MetadataHelper metadataHelper, RefactoredShardQueryConfiguration config, Query settings, String queryString,
    ...
    public static void configureTypeMappings(RefactoredShardQueryConfiguration config, IteratorSetting cfg, MetadataHelper metadataHelper,
                    boolean compressMappings) throws DatawaveQueryException {
        try {
     ...
     */
    protected CloseableIterable<QueryPlan> getFullScanRange(RefactoredShardQueryConfiguration config, JexlNode queryTree) {
        if (log.isTraceEnabled()) {
     ...
     * @return
     */
    private RangeStream initializeRangeStream(RefactoredShardQueryConfiguration config, ScannerFactory scannerFactory, MetadataHelper metadataHelper) {
        Class<? extends RangeStream> rstream;
        try {
            rstream = Class.forName(rangeStreamClass).asSubclass(RangeStream.class);
            
            RangeStream stream = rstream.getConstructor(RefactoredShardQueryConfiguration.class, ScannerFactory.class, MetadataHelper.class).newInstance(
                            config, scannerFactory, metadataHelper);
    ...
    protected boolean areSortedUIDsRequired(ASTJexlScript script, RefactoredShardQueryConfiguration config) {
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.QueryPlanner#maxRangesPerQueryPiece()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.QueryPlanner#setRules(java.util. Collection )
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.QueryPlanner#getRules()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.QueryPlanner#getPlannedScript()
     */
    @Override
    ...
    protected Multimap<String,Type<?>> configureIndexedAndNormalizedFields(MetadataHelper metadataHelper, RefactoredShardQueryConfiguration config,
    ...
    protected Multimap<String,Type<?>> configureIndexedAndNormalizedFields(Multimap<String,Type<?>> fieldToDatatypeMap, final Set<String> indexedFields,
                    final Set<String> normalizedFields, RefactoredShardQueryConfiguration config, ASTJexlScript queryTree) throws DatawaveQueryException,
        ...
        config.setQueryFieldsDatatypes(HashMultimap.create(Multimaps.filterKeys(fieldToDatatypeMap, new Predicate<String>() {
            
            @Override
            public boolean apply(String input) {
                return !normalizedFields.contains(input);
            }
        })));
        log.debug("IndexedFields Datatypes: " + config.getQueryFieldsDatatypes());
        
        config.setNormalizedFieldsDatatypes(HashMultimap.create(Multimaps.filterKeys(fieldToDatatypeMap, new Predicate<String>() {
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.QueryPlanner#getQueryIteratorClass()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see datawave.query.rewrite.planner.QueryPlanner#setQueryIteratorClass( java.lang.Class)
     */
    @Override
