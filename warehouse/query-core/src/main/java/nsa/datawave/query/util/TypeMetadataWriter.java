...
public class TypeMetadataWriter {
    ...
    public void writeTypeMetadataMap(Map<Set<String>,TypeMetadata> map, String name) throws Exception {
        ...
        try {
            ...
            oos.writeObject(map);
            log.debug("wrote the typeMetadataMap to hdfs at " + this.bridge.getUri() + " " + this.bridge.getDir() + "/" + this.bridge.getTempFileName());
        } finally {
            if (oos != null) {
                oos.close();
            }
        ...
        try {
            ...
            log.debug("renamed typeMetadataFile");
        } catch (Exception ex) {
            log.warn("got " + ex + " renaming typeMetadataFile");
        }
    }
