package nsa.datawave.query.language.parser.lucene;
...
import nsa.datawave.query.language.builder.lucene.AccumuloQueryTreeBuilder;
...
import nsa.datawave.query.language.processor.lucene.CustomQueryNodeProcessorPipeline;
import nsa.datawave.query.language.tree.FunctionNode;
import nsa.datawave.query.language.tree.HardAndNode;
import nsa.datawave.query.language.tree.NotNode;
import nsa.datawave.query.language.tree.SelectorNode;
import nsa.datawave.query.language.tree.SoftAndNode;
import nsa.datawave.query.search.FieldedTerm;
import nsa.datawave.query.search.RangeFieldedTerm;
...
public class LuceneQueryParser implements QueryParser {
    ...
    @Override
    public nsa.datawave.query.language.tree.QueryNode parse(String query) throws ParseException {
        query = query.replaceAll("\\u0093", "\""); // replace open smart quote 147
        query = query.replaceAll("\\u0094", "\""); // replace close smart quote 148
        
        nsa.datawave.query.language.tree.QueryNode parsedQuery = null;
        ...
        try {
            ...
            parsedQuery = (nsa.datawave.query.language.tree.QueryNode) builder.build(queryTree);
    ...
    private void extractFilters(nsa.datawave.query.language.tree.QueryNode node, Set<FieldedTerm> filters) {
        if (node instanceof FunctionNode) {
            ...
            List<nsa.datawave.query.language.tree.QueryNode> childrenList = node.getChildren();
        ...
        if (node instanceof SoftAndNode || node instanceof HardAndNode) {
            List<nsa.datawave.query.language.tree.QueryNode> evaluateNodes = new ArrayList<>();
            ...
            if (evaluateNodes.size() > 0) {
                ...
                for (nsa.datawave.query.language.tree.QueryNode n : evaluateNodes) {
            ...
            List<nsa.datawave.query.language.tree.QueryNode> children = node.getChildren();
            
            // If the first child of a NOT node might have filterable fields in it
            nsa.datawave.query.language.tree.QueryNode positiveNode = children.get(0);
            extractFilters(positiveNode, filters);
        }
    }
    
    private Set<FieldedTerm> extractAlwaysFilterNodes(Set<FieldedTerm> possibleFilters, List<nsa.datawave.query.language.tree.QueryNode> nodes) {
        ListIterator<nsa.datawave.query.language.tree.QueryNode> itr = nodes.listIterator(nodes.size());
        
        while (itr.hasPrevious() && nodes.size() > 0) {
            nsa.datawave.query.language.tree.QueryNode n = itr.previous();
            if (n instanceof FunctionNode) {
                FunctionNode fNode = (FunctionNode) n;
                possibleFilters.add((FieldedTerm) fNode.getQuery());
                itr.remove();
            } else {
                List<nsa.datawave.query.language.tree.QueryNode> children = n.getChildren();
                if (children.size() > 0) {
    ...
    private Set<FieldedTerm> extractFilters(List<nsa.datawave.query.language.tree.QueryNode> nodes) {
