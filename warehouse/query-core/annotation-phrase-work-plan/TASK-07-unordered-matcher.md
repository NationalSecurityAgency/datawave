...
- Added `UnorderedAnnotationMatcherTest`, including a table-driven three-value virtual-span matrix, ordinal-distance/timestamp-gap behavior, flatten modes, repeated terms, score thresholds, permutation deduplication, a configured four-component distance-4 span, overlap, and an explicit timeout-bounded stress test with many alternatives.
- Validation passed with `mvn -pl warehouse/query-core -am -DskipITs -Dtest=UnorderedAnnotationMatcherTest,OrderedAnnotationMatcherTest -Dsurefire.failIfNoSpecifiedTests=false test`: 13 tests, 0 failures, 0 errors. Checkstyle passed; output contained only pre-existing repository import-control warnings.
