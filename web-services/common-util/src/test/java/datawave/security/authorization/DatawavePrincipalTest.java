...
public class DatawavePrincipalTest {
    ...
    public void GetPrimaryUserTest() {
        ...
        Assertions.assertEquals(finalConnectionServerSubjectDn, dp.getPrimaryUser().getDn().subjectDN());
        
        // direct call from a user
        dp = new DatawavePrincipal(Lists.newArrayList(user));
        Assertions.assertEquals(userSubjectDn, dp.getPrimaryUser().getDn().subjectDN());
        ...
        Assertions.assertEquals(server1SubjectDn, dp.getPrimaryUser().getDn().subjectDN());
    ...
    public void GetProxyServersTest() {
        ...
        DatawavePrincipal dp = new DatawavePrincipal(Lists.newArrayList(finalServer));
        Assertions.assertEquals(null, dp.getProxyServers());
        ...
        dp = new DatawavePrincipal(Lists.newArrayList(server1, finalServer));
        Assertions.assertEquals(Arrays.asList(finalConnectionServerSubjectDn), dp.getProxyServers());
        
        // call from finalServer proxying initial caller server1 through server2
        dp = new DatawavePrincipal(Lists.newArrayList(server1, server2, finalServer));
        Assertions.assertEquals(Arrays.asList(server2SubjectDn, finalConnectionServerSubjectDn), dp.getProxyServers());
        
        // call from finalServer proxying initial caller server1 through server2 and server3
        dp = new DatawavePrincipal(Lists.newArrayList(server1, server2, server3, finalServer));
        Assertions.assertEquals(Arrays.asList(server2SubjectDn, server3SubjectDn, finalConnectionServerSubjectDn), dp.getProxyServers());
        
        // these tests are for cases where a UserType.USER appears anywhere in the proxiedUsers collection
        
        dp = new DatawavePrincipal(Lists.newArrayList(user, server1, server2, server3));
        Assertions.assertEquals(Arrays.asList(server1SubjectDn, server2SubjectDn, server3SubjectDn), dp.getProxyServers());
    ...
    public void GetNameTest() {
        // direct call from finalServer
        DatawavePrincipal dp = new DatawavePrincipal(Lists.newArrayList(finalServer));
        Assertions.assertEquals(joinNames(Lists.newArrayList(finalServer)), dp.getName());
        
        // direct call from user
        dp = new DatawavePrincipal(Lists.newArrayList(user));
        Assertions.assertEquals(joinNames(Lists.newArrayList(user)), dp.getName());
        
        // call from finalServer proxying initial caller server1
        dp = new DatawavePrincipal(Lists.newArrayList(server1, finalServer));
        Assertions.assertEquals(joinNames(Lists.newArrayList(server1, finalServer)), dp.getName());
        
        // call from finalServer proxying initial caller server1 through server2
        dp = new DatawavePrincipal(Lists.newArrayList(server1, server2, finalServer));
        Assertions.assertEquals(joinNames(Lists.newArrayList(server1, server2, finalServer)), dp.getName());
        
        // call from finalServer proxying initial caller server1 through server2 and server3
        dp = new DatawavePrincipal(Lists.newArrayList(server1, server2, server3, finalServer));
        Assertions.assertEquals(joinNames(Lists.newArrayList(server1, server2, server3, finalServer)), dp.getName());
        
        // these tests are for cases where a UserType.USER appears anywhere in the proxiedUsers collection
        
        // this first case would be very odd -- call from user proxying initial caller server1 through server2 through server3
        dp = new DatawavePrincipal(Lists.newArrayList(user, server1, server2, server3));
        Assertions.assertEquals(joinNames(Lists.newArrayList(user, server1, server2, server3)), dp.getName());
    ...
    public void OrderProxiedUsers() {
        ...
        // call from finalServer proxying initial caller server1
        Assertions.assertEquals(Lists.newArrayList(server1, finalServer), DatawavePrincipal.orderProxiedUsers(Lists.newArrayList(server1, finalServer)));
        
        // call from finalServer proxying initial caller server1 through server2
        Assertions.assertEquals(Lists.newArrayList(server1, server2, finalServer),
                        DatawavePrincipal.orderProxiedUsers(Lists.newArrayList(server1, server2, finalServer)));
        
        // call from finalServer proxying initial caller server1 through server2 and server3
        Assertions.assertEquals(Lists.newArrayList(server1, server2, server3, finalServer),
