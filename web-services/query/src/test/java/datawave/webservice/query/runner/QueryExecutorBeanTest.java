...
public class QueryExecutorBeanTest {
    ...
    public void setup() throws Exception {
        ...
        Class<QueryTraceCache> clazz = QueryTraceCache.class;
        Method method = clazz.getDeclaredMethod("init");
        method.setAccessible(true);
        method.invoke(traceCache);
    ...
    public void testTriple_Nulls() throws Exception {
        ...
        subject.hashCode();
        Assertions.assertTrue(!subject.equals(null), "Should not be equal");
        Assertions.assertTrue(!subject.equals(new Triple("test", null, null)), "Should not be equal");
    }
    ...
    // Assertions.assertNotNull(cachedRunningQuery);
    // RunningQuery rq2 = (RunningQuery) cachedRunningQuery;
    // Assertions.assertEquals(q, rq2.getSettings());
    //
    // }
    ...
    //
    // Object cachedRunningQuery = cache.get(q.getId().toString());
    // Assertions.assertNull(cachedRunningQuery);
    //
    // Assertions.assertEquals(predictions.toString(), response.getResult());
    // }
    //
    // // @Test
    // @SuppressWarnings("unchecked")
    ...
    // Assertions.assertEquals(queries.size(), returnedQueries.getLength());
    // }
    ...
    public void testBeginDateAfterEndDate() throws Exception {
        ...
        try {
            ...
            Assertions.fail(); // If doesn't throw exception, should fail
        } catch (BadRequestException e) {
            Assertions.assertEquals(DatawaveErrorCode.BEGIN_DATE_AFTER_END_DATE.toString(), e.getCause().getMessage());
        }
    }
        ...
        try {
            ...
            while (!initializeLooping.get()) {
                if (!createQuery.isAlive() && !initializeLooping.get()) {
                    Assertions.fail("createQuery thread died before reaching initialize: " + createQueryException[0]);
                }
                Thread.sleep(50);
            }
            ...
            Assertions.assertEquals(logic, pair.getFirst());
            Assertions.assertEquals(c, pair.getSecond());
            ...
            Assertions.assertNull(pair, "Still found an entry in the qlCache: " + pair);
