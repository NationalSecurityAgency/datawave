...
public class AnnotationManagerBean implements AnnotationManager {
    ...
    public AnnotationDataAccess<Annotation, Segment> initializeAnnotationService() throws Exception {
    ...
    public Response getAnnotationsByType(@PathParam("idType") String idType, @PathParam("id") String id, @PathParam("annotationType") String annotationType) {
        ...
        try {
            ...
            final AnnotationDataAccess<Annotation, Segment> annotationDataAccess = initializeAnnotationService();
            final List<Annotation> annotations = annotationDataAccess.getAllForType(internalId.getShard(), internalId.getDataType(), internalId.getUid(), annotationType);
            if (annotations.isEmpty()) {
                return jsonNotFound(String.format("No annotations found for identifier '%s:%s', internalId: '%s', of type '%s'", idType, id, internalId, annotationType));
            }
            return jsonOk(annotations);
        } catch (Exception e) {
            log.error("Internal error fetching annotations", e);
    ...
    public Response getAnnotation(@PathParam("idType") String idType, @PathParam("id") String id, @PathParam("annotationId") String annotationId) {
        try {
            ...
            final AnnotationDataAccess<Annotation, Segment> annotationDataAccess = initializeAnnotationService();
            final Optional<Annotation> annotations = annotationDataAccess.getAnnotation(internalId.getShard(), internalId.getDataType(), internalId.getUid(), annotationId);
            if (annotations.isEmpty()) {
                return jsonNotFound(String.format("No annotations found for identifier '%s:%s', internalId: '%s', with id '%s'", idType, id, internalId, annotationId));
            }
            return jsonOk(annotations);
        } catch (Exception e) {
            log.error("Internal error fetching annotations", e);
