...
FOLDER="batchLookupContent_$(date +%Y%m%d_%I%M%S.%3N)"
...
curl -s -D headers_0.txt -k -E ${TMP_PEM} \
    ...
    ${DATAWAVE_ENDPOINT}/lookupContentUUID -o lookupResponse.xml
...
if [ ! -z "$QUERY_ID" ]; then
    mv $FOLDER batchLookupContent_$QUERY_ID

    echo "$(date): Getting metrics for $QUERY_ID"
    echo "$(date): Getting metrics for $QUERY_ID" >> batchLookupContent_$QUERY_ID/querySummary.txt

    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID"
    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID" >> batchLookupContent_$QUERY_ID/querySummary.txt
fi
