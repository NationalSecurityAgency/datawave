...
public class PhoneNumber implements Serializable, Comparable<PhoneNumber> {
    ...
    private static PhoneNumber isValid(String number) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            currentSpanLength = 0;
            if (i > start
                            && (!isDigit(data[i - 1]) && !(data[i - 1] == ' ' || data[i] == ' ') && !((data[i - 1] == '+' && data[i] == '(') || (data[i - 1] == '(' && data[i] == '+')))) {
    ...
    private static String basicPhoneNumberCheck(String number) {
        ...
        for (int i = MIN_LENGTH; i < data.length; i++) {
            ...
            if (i < data.length - 1
                            && ((Character.isLetter(data[i + 1]) && data[i + 1] != 'x' && data[i] != 'X') || data[i + 1] == '_' || data[i + 1] == '='
                                            || data[i + 1] == '?' || data[i + 1] == '\\')) {
                continue;
            } else if (i < data.length - 2 && data[i + 1] == '/' && !isDigit(data[i + 2])) {
                continue;
                ...
                continue;
                            ...
                            && (data[start - 1] == '=' || data[start - 1] == '*' || data[start - 1] == '/' || data[start - 1] == '_' || data[start - 1] == '?'
