...
function prettyPrintJson() {
    local PY=$( which python3 || which python )
    if [ -n "${PY}" ] ; then
        echo "${1}" | (${PY} -c 'from __future__ import print_function;import sys,json;data=json.loads(sys.stdin.read()); print(json.dumps(data, indent=2, sort_keys=True))' || ${PY} -c 'import sys,json;data=json.loads(sys.stdin.read()); print(json.dumps(data, indent=2, sort_keys=True))')
        local exitStatus=$?
        echo
        if [ "${exitStatus}" != "0" ] ; then
           printRawResponse "${1}"
