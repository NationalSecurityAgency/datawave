...
Generate the password-less ssh key now?"
      then
         ...
         ssh-keygen -q -N "" -t rsa -f ~/.ssh/id_rsa || ( fatal "SSH config failed! Please configure manually" && exit 1 )
         ...
         if ! ssh-add; then
             eval "$(ssh-agent)" && ssh-add || warn "Error adding ssh key. SSH issues may cause Hadoop installation to fail" && exit 1
         fi
      fi
...
if hadoopIsInstalled ; then
    info "Hadoop is already installed"
    exit 0
fi

# bootstrap and verify that the two checksums match before installing.
bootstrapHadoop
verifyChecksum "${DW_HADOOP_DIST_URI}" "${DW_HADOOP_SERVICE_DIR}" "${DW_HADOOP_DIST_SHA512_CHECKSUM}" || exit 1

info "Installing Hadoop..."
[ -f "${DW_HADOOP_SERVICE_DIR}/${DW_HADOOP_DIST}" ] || ( fatal "Hadoop tarball not found" && exit 1 )

# Extract, set symlink, and verify...
mkdir "${DW_HADOOP_SERVICE_DIR}/${DW_HADOOP_BASEDIR}" || ( fatal "Failed to create Hadoop base directory" && exit 1 )
tar xf "${DW_HADOOP_SERVICE_DIR}/${DW_HADOOP_DIST}" -C "${DW_HADOOP_SERVICE_DIR}/${DW_HADOOP_BASEDIR}" --strip-components=1 || ( fatal "Failed to extract Hadoop tarball" && exit 1 )
( cd "${DW_CLOUD_HOME}" && ln -s "bin/services/hadoop/${DW_HADOOP_BASEDIR}" "${DW_HADOOP_SYMLINK}" ) || ( fatal "Failed to set Hadoop symlink" && exit 1 )
hadoopIsInstalled || ( fatal "Hadoop was not installed" exit 1 )
info "Hadoop tarball extracted and symlinked"

sshIsInstalled || ( fatal "It appears that ssh and/or sshd are not installed. Please correct this before continuing" && exit 1 )

# Write *-site.xml files...

if [ ! -z "${DW_HADOOP_CORE_SITE_CONF}" ] ; then
   writeSiteXml "${HADOOP_CONF_DIR}/core-site.xml" "${DW_HADOOP_CORE_SITE_CONF}" || ( fatal "Failed to write core-site.xml" && exit 1 )
...
if [ ! -z "${DW_HADOOP_HDFS_SITE_CONF}" ] ; then
   writeSiteXml "${HADOOP_CONF_DIR}/hdfs-site.xml" "${DW_HADOOP_HDFS_SITE_CONF}" || ( fatal "Failed to write hdfs-site.xml" && exit 1 )
...
if [ ! -z "${DW_HADOOP_MAPRED_SITE_CONF}" ] ; then
   writeSiteXml "${HADOOP_CONF_DIR}/mapred-site.xml" "${DW_HADOOP_MAPRED_SITE_CONF}" || ( fatal "Failed to write mapred-site.xml" && exit 1 )
...
if [ ! -z "${DW_HADOOP_YARN_SITE_CONF}" ] ; then
   writeSiteXml "${HADOOP_CONF_DIR}/yarn-site.xml" "${DW_HADOOP_YARN_SITE_CONF}" || ( fatal "Failed to write yarn-site.xml" && exit 1 )
...
if [ ! -z "${DW_HADOOP_CAPACITY_SCHEDULER_CONF}" ] ; then
   writeSiteXml "${HADOOP_CONF_DIR}/capacity-scheduler.xml" "${DW_HADOOP_CAPACITY_SCHEDULER_CONF}" || ( fatal "Failed to write capacity-scheduler.xml" && exit 1 )
...
# Format namenode
${HADOOP_HOME}/bin/hdfs namenode -format || ( fatal "Failed to initialize Hadoop" && exit 1 )
