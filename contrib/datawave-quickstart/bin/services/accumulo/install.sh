...
accumuloIsInstalled || bootstrapAccumulo && verifyChecksum "${DW_ACCUMULO_DIST_URI}" "${DW_ACCUMULO_SERVICE_DIR}" "${DW_ACCUMULO_DIST_SHA512_CHECKSUM}"
# If Zookeeper is not installed, bootstrap and verify that the two checksums match before installing.
zookeeperIsInstalled || bootstrapZookeeper && verifyChecksum "${DW_ZOOKEEPER_DIST_URI}" "${DW_ACCUMULO_SERVICE_DIR}" "${DW_ZOOKEEPER_DIST_SHA512_CHECKSUM}"
