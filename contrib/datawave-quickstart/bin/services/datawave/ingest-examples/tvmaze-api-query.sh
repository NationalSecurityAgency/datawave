...
if [ "${PRETTY}" == true ] ; then
    echo "${TVMAZE_RESPONSE_BODY}" | (python3 -c 'from __future__ import print_function; import sys,json; data=json.loads(sys.stdin.read()); print(json.dumps(data, indent=2, sort_keys=True))' || python -c 'import sys,json; data=json.loads(sys.stdin.read()); print(json.dumps(data, indent=2, sort_keys=True))')
else
