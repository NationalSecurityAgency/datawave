...
function bootstrapEmbeddedMaven() {
    if [ ! -f "${DW_MAVEN_SERVICE_DIR}/${DW_MAVEN_DIST}" ]; then
        ...
        DW_MAVEN_DIST="$( { downloadTarball "${DW_MAVEN_DIST_URI}" "${DW_MAVEN_SERVICE_DIR}" || downloadMavenTarball "datawave-parent" "gov.nsa.datawave.quickstart" "maven" "3.8.8" "${DW_MAVEN_SERVICE_DIR}"; } && echo "${tarball}" )"
    fi
