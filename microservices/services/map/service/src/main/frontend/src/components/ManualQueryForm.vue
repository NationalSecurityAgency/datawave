        ...
        <q-card-section class="q-py-none">
            <q-form>
                <div class="q-py-sm">
                    ...
                    <q-input v-model="manualQueryForm.query" label="Query">
                        <template v-slot:append>
                            <q-btn dense flat icon="edit" @click="editDialog=true; editDialogText=manualQueryForm.query" />
                ...
                <div class="q-py-sm">
                    ...
                    <div class="q-pt-md" style="display: flex; flex-direction: column; align-items: center">
                        <q-btn icon="add_circle" label="Add Geo Field" @click="manualQueryForm.fieldTypes.push({id: fieldNum++, field: '', type: ''} as FieldType)" />
                    </div>
                </div>
            </q-form>
            <q-card-actions align="right" class="text-primary">
                <q-btn flat label="Reset" @click="manualQueryForm.$reset(); fieldNum = 0;" />
    ...
    <q-dialog v-model="editDialog">
        <q-card style="min-width: 66%;">
            ...
            <q-card-actions align="right" class="text-primary">
                ...
                <q-btn flat label="Save" v-close-popup @click="manualQueryForm.query=editDialogText; editDialogText=''" />
            </q-card-actions>
        </q-card>
    </q-dialog>
</template>
...
function getFieldTypeMap() {
    ...
    if (element.field !== '') {
            ...
            if (fieldTypeMap.has(element.field)) {
                typeArray = fieldTypeMap.get(element.field)!;
            } else {
...
function submitQuery() {
    ...
    geoQueryFeatures.loadGeoFeaturesForQuery(manualQueryForm.query, getFieldTypeMap());
}
