...
FOLDER="count_$(date +%Y%m%d_%I%M%S.%N)"
...
curl -s -D headers_0.txt -k -E ${TMP_PEM} \
    ...
    ${DATAWAVE_ENDPOINT}/CountQuery/create -o createResponse.xml -w '%{http_code}\n' >> querySummary.txt
...
if [ ! -z "$QUERY_ID" ]; then
    mv $FOLDER count_$QUERY_ID

    echo "$(date): Getting metrics for $QUERY_ID"
    echo "$(date): Getting metrics for $QUERY_ID" >> count_$QUERY_ID/querySummary.txt

    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID"
    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID" >> count_$QUERY_ID/querySummary.txt
fi
