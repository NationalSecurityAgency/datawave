...
FOLDER="mapReduceQuery_$(date +%Y%m%d_%I%M%S.%N)"
...
curl -s -D headers_1.txt -k -E ${TMP_PEM} \
    ...
    --data-urlencode "outputFormat=TEXT" \
    ${DATAWAVE_ENDPOINT}/mapreduce/submit -o submitResponse.xml -w '%{http_code}\n' >> querySummary.txt
...
if [ ! -z "$JOB_ID" ]; then
    mv $FOLDER mapReduceQuery_$JOB_ID

    echo "$(date): Job status available at: ${DATAWAVE_ENDPOINT}/mapreduce/${JOB_ID}/list"
    echo "$(date): Job status available at: ${DATAWAVE_ENDPOINT}/mapreduce/${JOB_ID}/list" >> mapReduceQuery_$JOB_ID/querySummary.txt
fi
