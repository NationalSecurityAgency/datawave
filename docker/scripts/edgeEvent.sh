...
FOLDER="edgeEvent_$(date +%Y%m%d_%I%M%S.%3N)"
...
curl -s -D headers_0.txt -k -E ${TMP_PEM} \
    ...
    --data-urlencode "query=SOURCE:Jerry\\ Seinfeld SINK:Seinfeld TYPE:TV_SHOW_CAST RELATION:PERSON-SHOW" \
    ...
    ${DATAWAVE_ENDPOINT}/EdgeEventQuery/create -o createResponse.xml -w '%{http_code}\n' >> querySummary.txt
...
if [ ! -z "$QUERY_ID" ]; then
    mv $FOLDER edgeEvent_$QUERY_ID

    echo "$(date): Getting metrics for $QUERY_ID"
    echo "$(date): Getting metrics for $QUERY_ID" >> edgeEvent_$QUERY_ID/querySummary.txt

    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID"
    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID" >> edgeEvent_$QUERY_ID/querySummary.txt
fi
