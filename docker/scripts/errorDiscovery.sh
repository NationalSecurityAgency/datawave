...
FOLDER="errorDiscovery_$(date +%Y%m%d_%I%M%S.%N)"
...
curl -s -D headers_0.txt -k -E ${TMP_PEM} \
    ...
    ${DATAWAVE_ENDPOINT}/ErrorDiscoveryQuery/create -o createResponse.xml -w '%{http_code}\n' >> querySummary.txt
...
if [ ! -z "$QUERY_ID" ]; then
    mv $FOLDER errorDiscovery_$QUERY_ID
fi
