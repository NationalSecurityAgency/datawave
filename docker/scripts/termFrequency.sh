...
FOLDER="termFrequency_$(date +%Y%m%d_%I%M%S.%3N)"
...
curl -s -D headers_0.txt -k -E ${TMP_PEM} \
    ...
    --data-urlencode "begin=19500101 000000.000" \
    --data-urlencode "end=20161002 235959.999" \
    --data-urlencode "columnVisibility=PUBLIC" \
    --data-urlencode "query=jackie:19520920_0/tvmaze/-bb3qxp.e771of.e3f2gs" \
    ...
    ${DATAWAVE_ENDPOINT}/TermFrequencyQuery/create -o createResponse.xml -w '%{http_code}\n' >> querySummary.txt
...
if [ ! -z "$QUERY_ID" ]; then
    mv $FOLDER termFrequency_$QUERY_ID

    echo "$(date): Getting metrics for $QUERY_ID"
    echo "$(date): Getting metrics for $QUERY_ID" >> termFrequency_$QUERY_ID/querySummary.txt

    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID"
    echo "$(date): Metrics available at: ${METRICS_ENDPOINT}/id/$QUERY_ID" >> termFrequency_$QUERY_ID/querySummary.txt
fi
